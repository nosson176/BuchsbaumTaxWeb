<template>
  <Table class="h-auto shadow" @keydown.tab.prevent="onKeyDown">
    ...
    <template #body>
      <draggable :value="taxYearStateStatusDetails" v-bind="dragOptions" @start="startDrag" @end="onDrop">
        <transition-group type="transition" :name="transitionName">
          <TableRow v-for="(type, idx) in taxYearStateStatusDetails" :key="type.id" class="pr-1">
...
export default {
  ...
  computed: {
    ...
    isDefaultOrder() {
      return this.taxYearStateStatusDetails?.every((detail) => !detail.sortOrder)
    },
  },
  methods: {
    toggleEditable(id) {
    ...
    onDrop(evt) {
      const item = this.taxYearStateStatusDetails[evt.oldIndex]
      item.sortOrder = evt.newIndex + 1
      this.$api.updateValueType(this.headers, { valueId: item.id }, item)
      this.dragActive = false
    },
    resetOrder() {
      const item = this.taxYearStateStatusDetails[0]
      item.sortOrder = 0
      this.$api.updateValueType(this.headers, { valueId: item.id }, item)
    },
  },
