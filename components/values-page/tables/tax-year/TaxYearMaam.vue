<template>
    <Table class="h-auto shadow" @keydown.tab.prevent="onKeyDown">
        ...
        <template #body>
            <draggable :value="taxYearMaam" v-bind="dragOptions" @start="startDrag" @end="onDrop">
                <transition-group type="transition" :name="transitionName">
                    <TableRow v-for="(type, idx) in taxYearMaam" :key="type.id" class="pr-1"
...
const TABLE_TYPE = 'maam'

export default {
    name: 'TaxYearMaamTable',
    ...
    methods: {
        ...
        handleUpdate() {
            const value = Object.values(this.taxYearMaam).find((type) => type.id === this.editableId)
        ...
        onDrop(evt) {
            ...
            const list = Object.values({ ...this.taxYearMaam });
