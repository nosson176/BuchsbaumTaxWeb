<template>
    <Table class="h-auto shadow" @keydown.tab.prevent="onKeyDown">
        ...
        <template #body>
            <draggable :value="taxYearBasicPlus" v-bind="dragOptions" @start="startDrag" @end="onDrop">
                <transition-group type="transition" :name="transitionName">
                    <TableRow v-for="(type, idx) in taxYearBasicPlus" :key="type.id" class="pr-1"
                        :selected="isSelected(type)" @click="toggleSelected(type)">
                        ...
                        <div class="table-col w-full flex justify-between items-center"
                            @click="toggleEditable(type.id)">
                            <EditableInput v-model="type.value" :is-editable="isEditable(type.id)"
                                @input="debounceUpdate" @blur="onBlur" />
                        </div>
...
const TABLE_TYPE = 'basic_plus_pro'

export default {
    name: 'TaxYearBasicPlusTable',
    ...
    methods: {
        ...
        handleUpdate() {
            const value = Object.values(this.taxYearBasicPlus).find((type) => type.id === this.editableId)
        ...
        onDrop(evt) {
            ...
            const list = Object.values({ ...this.taxYearBasicPlus });
        ...
        resetOrder() {
            ...
            const list = [...this.taxYearBasicPlus];
            ...
            const item = this.taxYearBasicPlus[0];
            item.sortOrder = 0;
