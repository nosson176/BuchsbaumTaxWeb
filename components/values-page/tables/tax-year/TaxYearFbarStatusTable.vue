    ...
    <template #body>
      <draggable :value="taxYearFbarStatus" v-bind="dragOptions" @start="startDrag" @end="onDrop">
        <transition-group type="transition" :name="transitionName">
          <TableRow v-for="(type, idx) in taxYearFbarStatus" :key="type.id" class="pr-1" :selected="isSelected(type)"
...
export default {
  ...
  methods: {
    ...
    onDrop(evt) {
      ...
      const list = Object.values({ ...this.taxYearFbarStatus });
    ...
    resetOrder() {
      ...
      const list = [...this.taxYearFbarStatus];
