<template>
  <Table class="h-auto shadow" @keydown.tab.prevent="onKeyDown">
    ...
    <template #body>
      <draggable :value="taxYearFbarStatusDetails" v-bind="dragOptions" @start="startDrag" @end="onDrop">
        <transition-group type="transition" :name="transitionName">
          <TableRow v-for="(type, idx) in taxYearFbarStatusDetails" :key="type.id" class="pr-1">
...
const TABLE_TYPE = 'fbar_status_detail'

export default {
  name: 'TaxYearFbarStatusDetailsTable',
  ...
  computed: {
    ...
    isDefaultOrder() {
      return this.taxYearFbarStatusDetails?.every((detail) => !detail.sortOrder)
    },
  },
  methods: {
    toggleEditable(id) {
    ...
    handleUpdate() {
      const value = Object.values(this.taxYearFbarStatusDetails).find((type) => type.id === this.editableId)
    ...
    onDrop(evt) {
      const item = this.taxYearFbarStatusDetails[evt.oldIndex]
      item.sortOrder = evt.newIndex + 1
      this.$api.updateValueType(this.headers, { valueId: item.id }, item)
      this.dragActive = false
    },
    resetOrder() {
      const item = this.taxYearFbarStatusDetails[0]
      item.sortOrder = 0
      this.$api.updateValueType(this.headers, { valueId: item.id }, item)
    },
  },
