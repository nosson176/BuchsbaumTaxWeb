<template>
  <Table class="h-auto shadow" @keydown.tab.prevent="onKeyDown">
    ...
    <template #body>
      <draggable :value="taxYearTaxForms" v-bind="dragOptions" @start="startDrag" @end="onDrop">
        <transition-group type="transition" :name="transitionName">
          <TableRow v-for="(type, idx) in taxYearTaxForms" :key="type.id" class="pr-1">
...
const TABLE_TYPE = 'tax_form'

export default {
  name: 'TaxYearTaxFormsTable',
  ...
  computed: {
    ...
    isDefaultOrder() {
      return this.taxYearTaxForms?.every((form) => !form.sortOrder)
    },
  },
  methods: {
    toggleEditable(id) {
