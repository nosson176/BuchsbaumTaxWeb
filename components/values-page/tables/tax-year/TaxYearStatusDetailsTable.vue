<template>
  <Table class="h-auto shadow" @keydown.tab.prevent="onKeyDown">
    ...
    <template #body>
      <draggable :value="taxYearStatusDetails" v-bind="dragOptions" @start="startDrag" @end="onDrop">
        <transition-group type="transition" :name="transitionName">
          <TableRow v-for="(type, idx) in taxYearStatusDetails" :key="type.id" class="pr-1">
...
const TABLE_TYPE = 'tax_year_status_detail'

export default {
  name: 'TaxYearStatusDetailsTable',
  ...
  computed: {
    ...
    taxYearStatusDetails() {
      ...
      console.log(JSON.parse(JSON.stringify(this.valueTypes[TABLE_TYPE])))
  ...
  methods: {
    ...
    handleUpdate() {
      const value = this.taxYearStatusDetails.find((type) => type.id === this.editableId)
      if (value) {
    ...
    onDrop(evt) {
      ...
      const list = Object.values({ ...this.taxYearStatusDetails });
    ...
    resetOrder() {
      ...
      const list = [...this.taxYearStatusDetails];
      ...
      const item = this.taxYearStatusDetails[0];
      item.sortOrder = 0;
