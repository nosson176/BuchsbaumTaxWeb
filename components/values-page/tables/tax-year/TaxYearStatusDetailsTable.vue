...
export default {
  ...
  computed: {
    ...
    isDefaultOrder() {
      return this.taxYearStatusDetails?.every((detail) => !detail.sortOrder)
    },
  },
  methods: {
    toggleEditable(id) {
    ...
    handleUpdate() {
      const value = Object.values(this.taxYearStatusDetails).find((type) => type.id === this.editableId)
    ...
    onDrop(evt) {
      const item = this.taxYearStatusDetails[evt.oldIndex]
