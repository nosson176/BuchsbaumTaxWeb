    ...
    <template #body>
      <draggable :value="taxYearStateStatus" v-bind="dragOptions" @start="startDrag" @end="onDrop">
        <transition-group type="transition" :name="transitionName">
          <TableRow v-for="(type, idx) in taxYearStateStatus" :key="type.id" class="pr-1" :selected="isSelected(type)"
...
export default {
  ...
  methods: {
    ...
    onDrop(evt) {
      ...
      const list = Object.values({ ...this.taxYearStateStatus });
    ...
    resetOrder() {
      ...
      const list = [...this.taxYearStateStatus];
      ...
      const item = this.taxYearStateStatus[0];
      item.sortOrder = 0;
