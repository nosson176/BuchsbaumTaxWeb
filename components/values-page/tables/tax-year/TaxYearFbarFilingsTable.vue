<template>
  <Table class="h-auto shadow" @keydown.tab.prevent="onKeyDown">
    ...
    <template #body>
      <draggable :value="taxYearFbarFilings" v-bind="dragOptions" @start="startDrag" @end="onDrop">
        <transition-group type="transition" :name="transitionName">
          <TableRow v-for="(type, idx) in taxYearFbarFilings" :key="type.id" class="pr-1">
...
export default {
  ...
  computed: {
    ...
    isDefaultOrder() {
      return this.taxYearFbarFilings?.every((filing) => !filing.sortOrder)
    },
  },
  methods: {
    toggleEditable(id) {
