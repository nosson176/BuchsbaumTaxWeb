<template>
  <Table class="h-auto shadow" @keydown.tab.prevent="onKeyDown">
    ...
    <template #body>
      <draggable :value="contactTypes" v-bind="dragOptions" @start="startDrag" @end="onDrop">
        <transition-group type="transition" :name="transitionName">
          <TableRow v-for="(type, idx) in contactTypes" :key="type.id" class="pr-1">
...
export default {
  ...
  computed: {
    ...
    isDefaultOrder() {
      return this.contactTypes?.every((type) => !type.sortOrder)
    },
  },
  methods: {
    toggleEditable(id) {
