<template>
  <Table class="h-auto shadow" @keydown.tab.prevent="onKeyDown">
    ...
    <template #body>
      <draggable :value="sharedCategories" v-bind="dragOptions" @start="startDrag" @end="onDrop">
        <transition-group type="transition" :name="transitionName">
          <TableRow v-for="(type, idx) in sharedCategories" :key="type.id" class="pr-1">
...
const TABLE_TYPE = 'category'

export default {
  name: 'SharedCategoriesTable',
  ...
  computed: {
    ...
    isDefaultOrder() {
      return this.sharedCategories?.every((category) => !category.sortOrder)
    },
  },
  methods: {
    toggleEditable(id) {
    ...
    handleUpdate() {
      const value = Object.values(this.sharedCategories).find((type) => type.id === this.editableId)
