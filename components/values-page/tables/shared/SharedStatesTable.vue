<template>
  <Table class="h-auto shadow" @keydown.tab.prevent="onKeyDown">
    ...
    <template #body>
      <draggable :value="sharedStates" v-bind="dragOptions" @start="startDrag" @end="onDrop">
        <transition-group type="transition" :name="transitionName">
          <TableRow v-for="(type, idx) in sharedStates" :key="type.id" class="pr-1">
...
const TABLE_TYPE = 'state'

export default {
  name: 'SharedStatesTable',
  ...
  computed: {
    ...
    isDefaultOrder() {
      return this.sharedStates?.every((state) => !state.sortOrder)
    },
  },
  methods: {
    toggleEditable(id) {
    ...
    handleUpdate() {
      const value = Object.values(this.sharedStates).find((type) => type.id === this.editableId)
