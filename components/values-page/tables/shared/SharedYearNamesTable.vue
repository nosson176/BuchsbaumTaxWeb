<template>
  <Table class="h-auto shadow" @keydown.tab.prevent="onKeyDown">
    ...
    <template #body>
      <draggable :value="sharedYearNames" v-bind="dragOptions" @start="startDrag" @end="onDrop">
        <transition-group type="transition" :name="transitionName">
          <TableRow v-for="(type, idx) in sharedYearNames" :key="type.id" class="pr-1">
...
export default {
  ...
  computed: {
    ...
    isDefaultOrder() {
      return this.sharedYearNames?.every((name) => !name.sortOrder)
    },
  },
  methods: {
    toggleEditable(id) {
