    ...
    <template #body>
      <draggable :value="feeStatus" v-bind="dragOptions" @start="startDrag" @end="onDrop">
        <transition-group type="transition" :name="transitionName">
          <TableRow v-for="(type, idx) in feeStatus" :key="type.id" class="pr-1" :selected="isSelected(type)"
...
export default {
  ...
  methods: {
    ...
    onDrop(evt) {
      ...
      const list = Object.values({ ...this.feeStatus });
