<template>
  <Table class="h-auto shadow" @keydown.tab.prevent="onKeyDown">
    ...
    <template #body>
      <draggable :value="feeStatusDetails" v-bind="dragOptions" @start="startDrag" @end="onDrop">
        <transition-group type="transition" :name="transitionName">
          <TableRow v-for="(type, idx) in feeStatusDetails" :key="type.id" class="pr-1">
...
const TABLE_TYPE = 'fee_status_detail'

export default {
  name: 'FeeStatusDetailsTable',
  ...
  computed: {
    ...
    isDefaultOrder() {
      return this.feeStatusDetails?.every((status) => !status.sortOrder)
    },
  },
  methods: {
    toggleEditable(id) {
    ...
    handleUpdate() {
      const value = Object.values(this.feeStatusDetails).find((type) => type.id === this.editableId)
