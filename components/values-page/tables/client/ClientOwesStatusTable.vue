<template>
  <Table class="h-auto shadow" @keydown.tab.prevent="onKeyDown">
    ...
    <template #body>
      <draggable :value="clientOwesStatus" v-bind="dragOptions" @start="startDrag" @end="onDrop">
        <transition-group type="transition" :name="transitionName">
          <TableRow v-for="(type, idx) in clientOwesStatus" :key="type.id" class="pr-1">
...
const TABLE_TYPE = 'owes_status'

export default {
  name: 'ClientOwesStatusTable',
  ...
  computed: {
    ...
    isDefaultOrder() {
      return this.clientOwesStatus?.every((status) => !status.sortOrder)
    },
  },
  methods: {
    toggleEditable(id) {
    ...
    handleUpdate() {
      const value = Object.values(this.clientOwesStatus).find((type) => type.id === this.editableId)
