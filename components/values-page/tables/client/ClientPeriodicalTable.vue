<template>
  <Table class="h-auto shadow" @keydown.tab.prevent="onKeyDown">
    ...
    <template #body>
      <draggable :value="clientPeriodical" v-bind="dragOptions" @start="startDrag" @end="onDrop">
        <transition-group type="transition" :name="transitionName">
          <TableRow v-for="(type, idx) in clientPeriodical" :key="type.id" class="pr-1">
...
const TABLE_TYPE = 'periodical'

export default {
  name: 'ClientPeriodicalTable',
  ...
  computed: {
    ...
    isDefaultOrder() {
      return this.clientPeriodical?.every((periodical) => !periodical.sortOrder)
    },
  },
  methods: {
    toggleEditable(id) {
    ...
    handleUpdate() {
      const value = Object.values(this.clientPeriodical).find((type) => type.id === this.editableId)
