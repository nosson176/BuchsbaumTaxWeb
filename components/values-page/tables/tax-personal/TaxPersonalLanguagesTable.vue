<template>
  <Table class="h-auto shadow" @keydown.tab.prevent="onKeyDown">
    ...
    <template #body>
      <draggable :value="taxPersonalLanguages" v-bind="dragOptions" @start="startDrag" @end="onDrop">
        <transition-group type="transition" :name="transitionName">
          <TableRow v-for="(type, idx) in taxPersonalLanguages" :key="type.id" class="pr-1">
...
const TABLE_TYPE = 'language'

export default {
  name: 'TaxPersonalLanguagesTable',
  ...
  methods: {
    ...
    handleUpdate() {
      const value = Object.values(this.taxPersonalLanguages).find((type) => type.id === this.editableId)
    ...
    onDrop(evt) {
      ...
      const list = Object.values({ ...this.taxPersonalLanguages });
    ...
    resetOrder() {
      ...
      const list = [...this.taxPersonalLanguages];
      ...
      const item = this.taxPersonalLanguages[0];
      item.sortOrder = 0;
