<template>
  <div>
    ...
    <div class="mt-1 relative">
      ...
      <div v-else>
        <input ref="filter" v-model="computedValue" type="text"
          ...
          :placeholder="shownValue" @keyup="onInputKeyup($event.key)" @click="onInputClick" />
    ...
    <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100"
      ...
      <ul v-if="showOptions"
        ...
        tabindex="-1" :class="short ? 'max-h-20' : 'max-h-60'" role="listbox" aria-labelledby="listbox-label"
        aria-activedescendant="listbox-option-3" @blur="onBlur">
        <li v-for="(option, idx) in filteredOptions" id="listbox-option-0" ref="li" :key="option.id"
...
export default {
  ...
  computed: {
    ...
    computedValue: {
      ...
      //   return this.shownValue || this.value.map((item) => item.name || item.value)
      // },
      get() {
        // Ensure value is an array, convert from string or empty value if needed
        const normalizedValue = Array.isArray(this.value)
          ? this.value
          : (this.value ? [this.value] : []);

        return this.shownValue || normalizedValue.map((item) =>
      ...
      set(value) {
        ...
        if (returnValue.includes(value)) {
          returnValue.splice(returnValue.indexOf(value), 1)
        } else {
          returnValue.push(value)
        }
        this.$emit(events.input, returnValue)
      },
    },
    inputValue: {
      get() {
  ...
  methods: {
    ...
    isSelected(option) {
      if (option.value) {
        return this.computedValue.includes(option.name)
      }
    },
    onArrowDownPress() {
